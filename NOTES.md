...
- **2026-08-18** — **Hero rebuilt to the `template1.webp` direction + cohesion pass** (branch `feature/hero-template-upgrade`).
  - **Reference read.** template1 = light, airy hotel-booking hero: a rounded *floating card* inset from the page edges, soft sky→mint gradient, headline + two-line support + one dark pill CTA with a separate round ↗ button on the left, a large photograph bleeding into the right half, a tabbed **search panel** overlapping the card's bottom edge, then "eyebrow · big H2 · Explore-more pill + circle" section headers and an asymmetric card grid (tall left, two stacked right) with glass info bars.
  - **What we took:** the composition (card, left copy / right photo, overlapping panel), the CTA pill + circle pair, the section-header row, the light gradient hero, larger radii. **What we adapted:** colours stay ours (gradient = ocean at 7–15% + lagoon-mist, never a borrowed palette; CTA stays **gold**, not black); type stays Fraunces + Inter (headline is 3 lines of serif, accent line italic in **ocean** because gold text on a light ground fails AA); the "search" is a **quote-first planner** — a plain GET form to `/contact` with tabs = the five services (`content/services.ts` gained `shortName` + `quoteKey`), a per-tab primary field, dates, travellers, and one gold submit. `QuoteForm` now accepts the extra params, sanitises them, and opens at *Interests & budget* with a recap + Edit when service + usable dates arrive.
  - **Retired:** the aircraft sequence + `.hero-sky-clip` (built for the full-bleed Le Morne composition; the new hero has no sky corridor and the reference is calm), film grain, scroll cue, and the `CategoryCards` quick-nav (its five categories became the planner tabs). Header on Home is still fixed/transparent at the top but with **ink** text (the hero is light) and switches at 0.45×vh.
  - **Photo:** the same 1536px Le Morne upscale, now shown as a right-half inset (`lg`) with a left-edge mask (`@utility hero-photo-mask`), or as a 16:10 inset panel under the copy below `lg`. Single `<Image>` element for both via `lg:contents` on its wrapper so the alt text is not duplicated. Sharpness is acceptable at ~50vw; a ≥2000px original remains the #1 asset request.
  - **Cohesion:** `--radius-card` 16→20px site-wide; `ArrowCircle` (round icon link, decorative twin of a pill) used in the hero and the Featured header; feature `TourCard` image now grows to fill (`lg:flex-none` body) instead of leaving a void; `FeaturedTours` spacing `md` after the planner + trust strip; hero eyebrow = client tagline instead of repeating the wordmark.
  - **QA (headless Chrome, 1440×900 + 390×844, local prod build):** no console errors, no horizontal overflow, no broken images, one H1/page, planner tabs/keyboard/submit/prefill/edit paths verified, header states verified, all nav links 200. Note for future QA: full-page screenshots must scroll through the page first or `Reveal` sections capture as blank; new-headless Chrome cannot go below ~500px window width on Windows, so use puppeteer viewport emulation for mobile.
  - **Self-critique:** the hero now *belongs* to the site (own palette, own type, own signature line under the eyebrow/tabs) while clearly following the reference's bones. Open risks: (1) the reference's headline is 2 short lines; ours is 3 — good in serif but the copy could be tightened by the client; (2) the planner is a funnel accelerator, not a search — labels say "Get my quote" so it never promises availability; (3) interior `PageHeader` bands are still the dark full-bleed style, deliberately unchanged, but if the client wants the floating-card look everywhere that is the next visual step.
